...
const PokemonSetPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      if (filters.length > 0) {
        ...
        setData(data.items);
        console.log("FILTER RESPONSE:", resp.data);
        console.log("ITEMS COUNT:", data.items?.length, "TOTAL:", data.total);
        setTotal(data.total);
      } else {
  ...
  const columns = [
    ...
    { key: "set_name_en", label: "Tên bộ (EN)" },
    { key: "set_name_original", label: "Tên gốc" },
    { key: "series", label: "Series" },
    { key: "release_year", label: "Năm" },
    { key: "total_cards", label: "Số thẻ" },
    { key: "region", label: "Region" },
    { key: "series_order", label: "Thứ tự series" },
    {
      key: "created_at",
      label: "Ngày tạo",
      render: (row: PokemonSetRow) =>
